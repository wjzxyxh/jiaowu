...
from backend.models import Student, Course, StudentCourse, ClassHoursStats, Payment, TeacherHours, Teacher
from backend.utils.date_utils import get_current_month
...
def update_class_hours_stats(student_id, month=None, course_id=None):
    ...
    for cid in course_ids:
        ...
        for course_record in course_records:
            st = (course_record.status or '').strip()
            if st == '正常' or st == '':
                actual_hours += 1
            elif st == '请假':
